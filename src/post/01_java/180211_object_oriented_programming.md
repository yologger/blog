...
이 해시코드는 객체를 고유하게 구분하기 위해 사용된다. `HashMap`의 경우 데이터를 삽입할 때 `hashCode()`메소드의 반환값을 해시 테이블의 인덱스 주소값으로 사용한다. 따라서 충돌을 낮추고 성능을 향상시키기 위해 `hashCode()`를 잘 정의하는 것이 중요하다.

### equals()
메소드를 호출하는 객체와 인자로 전달받은 객체의 동등성을 판단한다. 다만 `Object`클래스를 상속하는 클래스에서는 `equals()`을 직접 구현해야한다.
``` java
