...
##  문자열 리터럴
Java에서는 보통 다음과 같이 `문자열 리터럴`로 String 타입의 변수를 초기화한다.
``` java
String str = "Hello";
```
사실 문자열은 원시 타입이 아니라 참조 타입이다. 따라서 문자열은 다음과 같이 `String`클래스의 생성자로 생성할 수 있다.
``` java
...
str = str + " world!";
```
문자열은 `불변 객체`이기 때문에 다음과 같이 기존 객체를 변경하는 것이 아니라
...
## StringBuffer 클래스
이처럼 문자열은 불변 객체이기 때문에 문자열 추가, 수정, 삭제 등의 연산이 빈번하면 메모리의 힙 영역에 객체가 여러 번 생성되어 성능에 영향을 주게 된다.

이러한 문제를 해결하기 위해 Java는 가변 객체인 `StringBuffer`와 `StringBuilder`를 도입했다.
