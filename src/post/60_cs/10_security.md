...
## DoS
- Denial of Service
- 서버에 과다한 요청을 보내서 네트워크 대역을 손상시키거나 서버를 과부화시켜 다른 요청을 받지 못하도록 하는 것
- DoS 공격은 하나의 호스트로 하나의 타겟을 공격한다.
...
### 해결방법
- 라우터나 리버스 프록시 서버를 배치하여 특정 IP만 요청할 수 있도록 제한할 수 있습니다.
- 또는 AWS의 IAM, 토큰 기반 인증 방식, Kubernetes의 ingress 기능 등을 사용할 수 있습니다.

## IP Spoofing
- TCP 3-way handshaing 과정에서 제 3자가 서버의 `SYN/ACK`를 가로챈 후 클라이언트의 IP로 가짜 ACK를 보내서 세션을 연결하는 방법
...
- 클라이언트는 데이터를 전송하기 전 SSL Shaking을 통해 서버로 부터 공개키를 발급받는다.
- 이후 공개키로 데이터를 암호화하여 전송하기 때문에 데이터가 유출되어도 암호화되어있기 떄문에 기밀성이 보장된다.
- HTTPS, VPN 등에 사용된다.
...
- Public Internet 망에 가상의 네트워크를 구축하고, SSL를 적용하여 기밀성을 보장한다.
...
- 입력 문자열로 출력 문자열을 찾을 수 있으나, 출력 문자열로는 입력 문자열을 찾을 수 없다는 `일방향성`을 가져야 한다.
...
## 전자서명
- 전자서명은 `무결성(위변조 방지)`, `송신자 인증`, `송신 부인 방지`
- 송신자는 평문으로 해시값을 만든다.
- 송신자는 평문+해시값을 개인키로 암호화하여 암호문을 전송한다.
- 수신자는 암호문을 공개키로 복호화하여 평문+해시값을 얻는다.
- 수신자가 공개키로 복호화에 성공했다는 것은 송신자가 제대로 된 개인키로 암호화했다는 것을 뜻하기 때문에 송신자 인증과 송신 부인 방지에 성공한다.
- 수신사는 평문과 해시값을 비교하여 무결성(위변조 방지)을 확인할 수 있다.
...
- JSP, mustache 같은 서버 사이드 렌더링에서는 XSS Filter와 같은 필터를 적용하여 입력 데이터를 검증한다.
