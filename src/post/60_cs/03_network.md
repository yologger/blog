...
- `물리 계층`: 데이터를 전기적 신호로 변환하고 전기적 신호를 어떻게 인식할 것인지를 정의한 계층. 비트, 전기적 신호가 이 계층이 포함된다. 리피터, 허브가 이 계층에 포함된다.
- `데이터 링크 계층`: 인접한 두 노드 간 데이터 속도제어, 순서제어, 오류제어를 하는 계층. 비트가 제대로 도착했는지를 제어한다. 브릿지와 스위치가 이 계층에 포함되며 기기마다 고유하게 할당되는 MAC 주소로 기기를 구분한다.
- `네트워크 계층`: 송신 측에서 수신 측까지 패킷을 전달하기 위한 계층. IP를 통해 `라우팅`하는 계층이며, 라우터가 이 계층에 포함된다.
- `전송 계층`: 송신 측과 수신 측의 신뢰성 있는 데이터 전송을 담당하는 계층. TCP와 UDP가 이 계층에 포함된다.
- `세션 계층`: 애플리케이션 레벨의 세션을 관리하는 계층
- `프리젠테이션 계층`: 데이터 표현 차이를 해결해주는 계층. 서로 다른 형식을 변환하거나 암호화/복호화, 압축을 담당하는 계층
...
### TCP
- 연결지향형 프로토콜
- 먼저 `3-way Handshaking`을 통해 연결을 만들고 데이터를 전송
- 수신측에서 패킷의 오류를 검사하고 오류가 있으면 재전송을 요청하기 때문에 신뢰성이 보장된다.
- 신뢰성을 필요로하는 어플리케이션에서 사용한다.

### TCP 3-way Handshaking
- 신뢰성있는 데이터 전송을 위해 데이터를 전송하기 전 세션을 수립하는 것
- 네 번째 Handshake로 연결을 종료한다.

### UDP
- 비연결지향형 프로토콜
- 데이터 수신을 보장하지 않지만 간단하고 빠르기 때문에 스트리밍같은 서비스에 사용된다.

## DNS
- `Domain Name Service`의 약자
- `IP 주소`를 `www.google.com`처럼 기억하기 쉬운 URL 주소로 변환해주는 것
...
|이름|설명|
|------|---|
|`GET`|서버에서 데이터를 가져올 때 사용하는 메서드. `Query Parameter`와 `Path Variable`로 값을 전달할 수 있다.|
|`POST`|서버에 리소스를 생성할 때 사용하는 메서드. `Rest API`에서는 `json`이나 `xml` 형태의 데이터를 `Body`에 담아 보낸다.|
...
- `4xx`: 클라이언트 에러
    ...
    - `401(Unauthorized)`: 인증이 안되었을 때 사용된다. 토큰 기반 로그인에서 아이디 또는 비밀번호가 잘못되었을 때 응답으로 사용할 수 있다.
    - `403(Forbiddend)`: 인증은 되었으나 권한이 없을 때 사용한다. 일반 사용자가 관리자의 리소스를 요청했을 때 응답으로 사용할 수 있다.
...
- 세션을 유지하는 것은 서버에 부담을 줄 수 있기 때문에 요즘에는 토큰 기반의 인증도 많이 사용한다.

## JWT
- 특정 정보 사용하여 검증 가능하고 만료 기한이 있는 문자열을 생성할 수 있다.
...
- 구글, 카카오 같은 간편 로그인도 `OAuth2` 기반의 인증
- 순서
    - 어플리케이션을 구글, 카카오 같은 제 3자에게 등록하고 `리다이렉션 URL`과 `API Key`를 발급받는다.
    - 개발자는 어플리케이션에 제 3자에게 발급받은 `API Key`를 등록한다.
...
- 실제로는 브라우저가 호스트, 프로토콜, 포트번호까지 일치해야한다.

## CORS
- `SOP`문제를 해결하기 위한 방법이 `CORS(교차 출처 리소스 공유)`
- 클라이언트와 서버가 정해진 헤더를 통해 교차 출처 리소스를 허용할지 결정하는 스펙
- `CORS`는 백엔드에서 처리해야하며, `Access-Control-Allow-Origin` 헤더에 적절한 정보를 실어 응답하도록 구현하면 됩니다.
...
## 이더넷과 맥주소
`MAC 주소`는 네트워크 인터페이스 카드에 고유하게 할당되는 번호. 
...
## 블록체인
데이터를 여러 호스트에 복제, 분산 저장하여 위변조를 방지하는 기술
