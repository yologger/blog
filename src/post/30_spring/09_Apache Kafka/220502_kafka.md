...
## Kafka 사용해보기
이제 Kafka의 메시지 브로커 기능을 사용해보자. 우선 `Topic`이라는 것을 생성해야한다. Zookeeper, Kafka 서버가 구동되는 상태에서 다음과 같이  `test-topic`이라는 Topic을 생성한다.
```
$ cd /usr/local/Cellar/kafka/3.1.0/bin
...
## Message Broker
이처럼 `메시지 브로커(Message Broker)`는 <u>Producer가 생성한 데이터를 Consumer에게 전달하는 시스템이다.</u>. 메시지 브로커는 내부에 큐(Queue)를 유지하여 데이터 전달 순서를 보장하기 때문에 `메시지 큐(Message Queue)`라고도 한다. 메시지 브로커 제품군에는 `Apache Kafka` 외에도 `RabbitMQ`, `Redis` 등이 있다.

## Kafka의 구성요소
Kafka는 여러 요소로 구성된다.

### Kafka 서버
`Kafka 서버`는 메시지 브로커 기능을 제공하며, `Broker 서버`라고도 한다.

`ZooKeeper` 1대, `Broker 서버` 1대로 이루어진 클러스터는 다음과 같이 구축할 수 있다.
...
### Kafka 클러스터
전송되는 메시지가 많을 경우 하나의 Kafka 서버로 감당하기 힘들 수 있다. Kafka 서버 여러 대를 하나로 묶어 `Kafka 클러스터`를 구축하면 더 많은 트래픽을 감당할 수 있다. 
