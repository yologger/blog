...
# AOP
<b>`AOP(Aspect Oriented Programming)`</b>은 공통 기능을 추출하여 별도의 모듈로 만드는 것이다. 이를 통해 모든 컴포넌트에 산재했는 공통 기능을 비즈니스 코드와 분리하여 관리할 수 있다. `AOP`는 주로 로깅, 트랜잭션, 보안 등의 공통 기능을 뽑아내는데 사용한다.
