...
스프링에서는 크게 두 가지의 외부 설정 방법을 제공한다.
- `@PropertySource`
...
외부 설정파일은 다른 `application.properties`를 포함할 수도 있다. `spring.profiles.include`를 사용하면 된다.
...
테스트 환경에서도 `appication-{profile}.properties`형태로 파일을 분리할 수 있다. `@ActiveProfiles` 어노테이션으로 활성화시킬 프로파일을 지정하면 된다.
``` properties
